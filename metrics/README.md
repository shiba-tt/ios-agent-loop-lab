...
- **追記(PR 内)**: タスクの PR 自身が 1 行追記する(PR テンプレートのチェック項目)。この時点では `outcome=open`、`human_touches` は空か 0 で良い
- **確定(マージ後)**: `outcome` と `human_touches` はマージ後にしか確定しないため、次にリポジトリを触るタスクか週次 L4 がまとめて確定する
- `human_minutes` はエージェントからは観測できない — 週次 L4 で人間が概算を記入する(空欄のままでも可)
- 正確さより継続を優先。概算で良い
